...
const ImageSlider = ({ 
  ...
  return (
    <div 
      className={`relative w-full h-125 lg:h-150 overflow-hidden shadow-2xl group ${className}`}
    ...
    >
                  ...
                  >
                    ...
                    <motion.h1
                      ...
                      className={`font-bold text-white leading-[1.1] tracking-tight ${
                        slides[currentIndex].isMainSlide 
                          ? 'text-4xl md:text-5xl lg:text-6xl xl:text-7xl mb-5' 
                          : 'text-4xl md:text-5xl lg:text-6xl xl:text-7xl mb-4 lg:mb-6'
                      }`}
                    >
                    ...
                    <motion.p
                      ...
                      className={`text-gray-200 font-light ${
                        slides[currentIndex].isMainSlide 
                          ? 'text-sm md:text-base lg:text-lg mb-8 max-w-xl leading-relaxed' 
                    ...
                    {slides[currentIndex].isMainSlide && slides[currentIndex].stats && (
                      <motion.div
                        ...
                        className="grid grid-cols-3 gap-3 md:gap-4 mb-8 max-w-xl"
                      >
                        {slides[currentIndex].stats.map((stat, idx) => (
                          <div key={idx} className="bg-black/30 backdrop-blur-md border border-white/20 rounded-2xl p-4 md:p-5 hover:bg-black/40 transition-all">
                            <div className="text-2xl md:text-3xl lg:text-4xl font-bold text-white mb-1.5">{stat.value}</div>
                            <div className="text-[9px] md:text-[10px] lg:text-xs text-gray-300 font-semibold uppercase tracking-wider leading-tight">{stat.label}</div>
                          </div>
                        ))}
                      </motion.div>
                    )}
                    ...
                    >
                      <button className="group px-7 md:px-9 lg:px-10 py-3 md:py-3.5 lg:py-4 bg-[#4A93C4] hover:bg-[#3b7ba8] text-white font-semibold text-sm md:text-base rounded-full transition-all duration-300 hover:shadow-2xl hover:shadow-[#4A93C4]/30 hover:scale-105 active:scale-95 flex items-center gap-2">
                        {slides[currentIndex].isMainSlide ? 'Request Cleanroom Quote' : 'Explore Solutions'}
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="relative w-full h-full container mx-auto px-4 lg:px-8">
          ...
          <motion.button
            ...
            className="pointer-events-auto absolute left-4 lg:left-8 top-1/2 -translate-y-1/2 p-4 bg-white/10 backdrop-blur-md hover:bg-white/20 text-white rounded-full transition-all duration-300 hover:scale-110 active:scale-95 border border-white/20 group"
          ...
          <motion.button
            ...
            animate={{ opacity: isHovered ? 1 : 0, x: isHovered ? 0 : -20 }}
            transition={{ duration: 0.3 }}
            className="pointer-events-auto absolute right-4 lg:right-8 top-1/2 -translate-y-1/2 p-4 bg-white/10 backdrop-blur-md hover:bg-white/20 text-white rounded-full transition-all duration-300 hover:scale-110 active:scale-95 border border-white/20 group"
      ...
      {/* Progress Bar */}
      <motion.div
        className="absolute top-0 left-0 h-1 bg-[#4A93C4] z-30"
        initial={{ width: '0%' }}
        animate={{ width: '100%' }}
      ...
      {/* Slide Counter */}
      <div className="absolute top-8 right-8 z-20 px-4 py-2 bg-black/30 backdrop-blur-md rounded-full border border-white/20">
